...
class FaustWasm2ScriptProcessor {
  ...
  getNode(dspInstance, audioCtx, bufferSize) {
    ...
    sp.remap = (v, mn0, mx0, mn1, mx1) =>
      ((v - mn0) / (mx0 - mn0)) * (mx1 - mn1) + mn1;
    ...
    sp.audio_heap_ptr_outputs =
      sp.audio_heap_ptr_inputs + sp.numIn * sp.ptr_size;

    // Setup buffer offset
    sp.audio_heap_inputs = sp.audio_heap_ptr_outputs + sp.numOut * sp.ptr_size;
    sp.audio_heap_outputs =
      sp.audio_heap_inputs + sp.numIn * bufferSize * sp.sample_size;
    ...
    sp.update_outputs = () => {
      if (
        ...
        sp.outputs_items.forEach((item) =>
          sp.output_handler(
            item,
            sp.factory.getParamValue(sp.dsp, sp.pathTable[item])
          )
        );
      }
    };
    ...
    sp.parse_group = (group) =>
      group.items ? sp.parse_items(group.items) : null;
    ...
    sp.initAux = () => {
      ...
      if (sp.numIn > 0) {
        ...
        for (let i = 0; i < sp.numIn; i++) {
          sp.HEAP32[(sp.ins >> 2) + i] =
            sp.audio_heap_inputs + bufferSize * sp.sample_size * i;
        }
        // Prepare Ins buffer tables
        const dspInChans = sp.HEAP32.subarray(
          sp.ins >> 2,
          (sp.ins + sp.numIn * sp.ptr_size) >> 2
        );
        for (let i = 0; i < sp.numIn; i++) {
      ...
      if (sp.numOut > 0) {
        ...
        for (let i = 0; i < sp.numOut; i++) {
          sp.HEAP32[(sp.outs >> 2) + i] =
            sp.audio_heap_outputs + bufferSize * sp.sample_size * i;
        }
    ...
    sp.instanceInit = (sampleRate) =>
      sp.factory.instanceInit(sp.dsp, sampleRate);
    ...
    sp.instanceConstants = (sampleRate) =>
      sp.factory.instanceConstants(sp.dsp, sampleRate);
    ...
    sp.getParamValue = (path) =>
      sp.factory.getParamValue(sp.dsp, sp.pathTable[path]);
